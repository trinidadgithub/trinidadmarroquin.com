...
A connected or host-backed virtual CD-ROM can become operational noise during VM maintenance. It can trigger device-lock prompts, confuse vMotion or storage work, and leave operators answering vCenter questions that have nothing to do with the actual change. If the VM is already locked by storage or vMotion tasks, clear the task contention first; see [vSphere CSI CNS ExtendVolume Triage](/field-notes/vsphere-csi-cns-extendvolume-triage/). For the incident-level journal on why CD-ROM cleanup should be treated as Kubernetes maintenance, see [Journal: When Storage Triage Turns Into Platform Maintenance](/posts/vsphere-csi-cdrom-maintenance-journal/). If live VM reconfigure causes Kubernetes symptoms, use a power-cycle-first workflow with role gates instead; see [vSphere CD-ROM Cleanup With Power-Cycle Gates](/field-notes/vsphere-cdrom-powercycle-maintenance-gates/).
